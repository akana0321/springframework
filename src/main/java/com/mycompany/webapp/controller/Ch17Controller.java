...
public class Ch17Controller {
	...
	public String join(Ch14Member member, Model model) {
		...
		/* 이미 memberService.join에서 암호화 처리를 하기 때문에 주석처리 함, 이런 방식이라는 것만 알아두기
		// 패스워드 암호화
		String mpassword = member.getMpassword();
		PasswordEncoder passwordEncoder = PasswordEncoderFactories.createDelegatingPasswordEncoder();
		mpassword = passwordEncoder.encode(mpassword); // {bcrypt} 암호화
	...
	public String userInfo(Authentication authentication) {
		...
		*/
		...
		jsonObject.put("mrole", authorityList);	// 배열 형태로 전송됨
